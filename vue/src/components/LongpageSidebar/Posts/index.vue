  ...
  <div class="h-100 bg-light">
    <div class="h-100 d-flex flex-column">
      <div class="p-3 bg-white">
        <div class="row pr-3">
          <h3 id="posts-sidebar-heading" class="col m-0">
            {{ $t("sidebar.tabs.posts.heading") }}
          </h3>
          <div class="col-auto px-0">
            <a href="javascript:void(0)" @click="toggleFormShown('filter')">
              <i
          ...
          <div class="col-auto px-0">
            <a href="javascript:void(0)" @click="toggleFormShown('sorting')">
              <i
        ...
        <hr v-show="formShown !== null" class="my-3" />
        <filter-form v-show="formShown === 'filter'" />
        <div v-show="formShown === 'sorting'">
          <select v-model="selectedThreadSortingOption" class="custom-select">
            <option
        ...
        <div v-if="selectedThreads.length" class="row mt-2">
          <div class="col">
            <p class="mb-1">
      ...
      <p v-else-if="threadsFiltered" class="p-3">
...
export default {
  ...
  computed: {
    ...mapGetters("post", [
      ...
      GET.SELECTED_THREAD_SORTING_OPTION,
      GET.THREADS,
      GET.THREAD_SORTING_OPTIONS,
    ]),
    ...mapGetters("post", { threadsFiltered: GET.THREADS_FILTERED }),
  ...
  mounted() {
      ...
      this.selectedThreads =
        type === AnnotationType.POST
          ? (this.threads || []).filter(
              (t) =>
                selection.findIndex(
                  (annotation) => annotation.id === t.annotationId,
                ) >= 0,
            )
          : [];
    });
    EventBus.subscribe("page-ready", () => {
      EventBus.publish(SidebarEvents.CHANGE_BADGES, {
        type: SidebarTabKeys.POSTS,
        count: (this.threads || [])
          .map((t) => t.posts.filter((p) => !p._readByUser).length)
          .reduce((p, a) => p + a, 0),
        title: "Neue Anmerkungen",
      });
    });
  },
  ...
  methods: {
    ...
    ...mapMutations("post", [MUTATE.SELECTED_THREAD_SORTING_OPTION]),
    resetFilter() {
