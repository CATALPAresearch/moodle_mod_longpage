...
export default {
  ...
  props: {
    contentsContainerSelector: {
      type: String,
      default: toIdSelector(LONGPAGE_CONTENT_ID),
    },
    headingSelectors: {
      type: Array,
      default: () => ["h3", "h4"],
  ...
  computed: {
    allHeadingsSelector() {
      return this.headingSelectors
        .map((hSelector) =>
          [this.contentsContainerSelector, hSelector].join(" "),
        )
        .join(", ");
    },
  },
  ...
  methods: {
    initHIntersectionObserver() {
      ...
      const intersectionObserver = new IntersectionObserver(
        (entries) => {
          const lastIntersectingEntry = findLast(
            entries,
            (e) => e.isIntersecting,
          );
          if (lastIntersectingEntry) {
            this.activeTOCEntryIndex = this.tocEntries.findIndex(
              (e) => e.hId === lastIntersectingEntry.target.id,
            );
            ...
            document
              .querySelector(
                `a.nav-link[href='#h-in-toc-${this.activeTOCEntryIndex}']`,
              )
          ...
          if (lastEntry.boundingClientRect.y > root.getBoundingClientRect().y) {
            this.activeTOCEntryIndex =
              this.tocEntries.findIndex((e) => e.hId === lastEntry.target.id) -
              1;
            if (this.activeTOCEntryIndex === -1) {
              return;
            }
      ...
      this.tocEntries
        .map((entry) => entry.hEl)
        .forEach((targetEl) => intersectionObserver.observe(targetEl));
    },
    initTOC() {
      document
        .querySelectorAll(this.allHeadingsSelector)
        .forEach((el, index) => {
          const hId = `h-in-toc-${index}`;
          const hLvl = this.headingSelectors.findIndex((selector) =>
            el.matches(selector),
          );
